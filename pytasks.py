...
def game(start, finish):
    ...
    try:
        if int(pc_cohice) == int(user_choice):
            ...
            return ('Fucking shit, you lose. Come back later!')
    except ValueError:
...
def dec_to_bin_rec(digit):
    ...
    if digit < 0:
        return ('Argument should be positive')
    elif digit == 0:
        return ''
    elif digit >=1:
        return dec_to_bin_rec(digit // 2) + str(digit % 2)
...
def ship(digit):
    ...
    #print(x,y)                      #Uncomment to see computer random choice
    pc_matrix = matrix
    pc_matrix[x-1][y-1] = 'X'
...
#Task_10
print(char_freq('asssassddddd   asd'))

# #Task_11
print(dec_to_bin_rec(2))

print(dec_to_bin_v2(2))
